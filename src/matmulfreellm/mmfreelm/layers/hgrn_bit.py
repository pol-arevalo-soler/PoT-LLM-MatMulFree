...
from mmfreelm.modules import FusedRMSNormGated, ShortConvolution
...
class HGRNBitAttention(nn.Module):
    ...
    def __init__(
        ...
        self.i_proj = FusedBitLinear(hidden_size, self.input_dim, bias=False)
        self.f_proj = FusedBitLinear(hidden_size, self.input_dim, bias=False)
        self.g_proj = FusedBitLinear(hidden_size, self.input_dim, bias=False)
        ...
        self.o_proj = FusedBitLinear(self.input_dim, hidden_size, bias=False)
    ...
    def _initialize_weights(self, module):
        if getattr(module, "_is_hf_initialized", False):
            return
        if isinstance(module, (nn.Linear, FusedBitLinear)):
            #nn.init.xavier_uniform_(module.weight, gain=2 ** -2.5)
            nn.init.kaiming_normal_(module.weight, nonlinearity='selu')
